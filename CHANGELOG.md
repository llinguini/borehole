...
- CLI error output is now a single clean line (red `✗ <message>`) instead of
  `anyhow`'s default `Error: ... Caused by: ...` dump. Set `BOREHOLE_DEBUG` to
  also print the underlying cause chain for troubleshooting.
